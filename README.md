...
input_text = message original de l’utilisateur (transmis via la sortie category_2_result du premier routeur)
...
Sortie category_1_result → correspond à Urgent

Sortie category_2_result → Non urgent

Note : Le deuxième routeur est configuré avec les routes "Urgent" / "Non urgent", mais son custom_prompt est vide. Il utilise donc le prompt par défaut.

4.2 Création de la page Notion (GroqModel-eAA9p + NotionPageCreator-3vDjw)
GroqModel-eAA9p reçoit le message de l’utilisateur et le transforme en JSON structuré pour Notion (titre, description, état).
...
4.5 Affichage (ChatOutput-4EJNY) et sauvegarde (SaveToFile-XdLPC)
Le message de confirmation est affiché dans le chat et sauvegardé dans un fichier local (/tmp/logss).
...
Un LLM détermine s’il s’agit d’une FAQ ou d’une demande de ticket.

Si FAQ : extraction de mots-clés → recherche dans la base Notion → extraction de la réponse → affichage.

Si ticket : second niveau d’urgence → création d’une page Notion → message de confirmation fixe → affichage.

Toutes les étapes sont journalisées dans des fichiers texte locaux.
